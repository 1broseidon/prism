...
Authentication checks read the server's published capabilities; they do not register OAuth clients or start sign-in. Registration happens when you explicitly choose **Sign in**, and a provider can still refuse it. Remote HTTP requests do not follow redirects, so custom API keys and request contents stay at the requested endpoint. If a server redirects, configure its final MCP URL directly.
...
**Repeated registrations from other clients.** Some clients pick a new loopback callback port each time they start. When the client name, registration source and callback match an existing generic agent except for that port, Prism offers a possible connection to that agent. Matching metadata does not prove identity. **Add connection** shares the agent's posture and rules while keeping its other connections signed in. **Keep as a separate agent** starts with First use and no inherited rules. **Replace** lets you choose one old connection to sign out and remove after the save succeeds. A refused, cancelled or failed-save replacement leaves the old connection usable.

Callback schemes, hosts, paths and queries remain distinct; ambiguous matches and registrations with multiple callback URLs get separate requests. Existing denials are preserved. Unused registrations and never-approved requests can expire after 24 hours, checked at startup and registration. Cleanup keeps manual and harness records, decisions, configured rules or posture, token history and active requests. An expired token alone is not grounds to delete an approved agent or its history.
