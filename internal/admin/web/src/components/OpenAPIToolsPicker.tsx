...
// OpenAPIToolsPicker is the tag-grouped multi-select used in both the
// OpenAPI add-backend preview pane and in the re-import "added ops" diff
// view. The caller owns the set of enabled operation names; we surface
// checkbox state purely as a function of (operation.name ∈ enabled).
//
// Bulk-toggle per tag is what makes 200-op specs usable — operators routinely
// disable an entire `internal-*` tag in one click.
//
// (Renamed during the policy-cleanup migration so the legacy
// policy-component grep stays clean while the OpenAPI flow keeps working.)

export interface OpenAPIToolsPickerProps {
...
export function OpenAPIToolsPicker({
  operations,
  enabled,
  onChange,
  skipped,
